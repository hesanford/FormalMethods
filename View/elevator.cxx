...
vector<Lift> lifts(NUMLIFTS);
vector<Floor> floors(NUMFLOORS);
int main() {
    for(int i = 0; i < lifts.size() - 1; i++) {
        lifts[i] = new Lift(LIFTSTARTFLOOR[i], LIFTPASSENGERS[i], LIFTSTARTDIR[i], LIFTBUTTONS[i]);
    }
    for(int i = 0; i < floors.size() - 1; i++) {
        floors[i] = new Floor(FLOORPASSENGERS[i], FLOORBUTTONS[i]);
    }
    make_window();
    Fl::run();
    return 0;
