...
int Controller::pickLift(int callingFloor, int callingDir){
    ...
    */
    for (int i = 0; i < (int)model.lifts.size(); i++){
...
void Controller::run(){
   ...
   for (int i = 0; i < (int)model.CallingDirList.size(); i++){
        ...
        pickedLift.setID(liftID);
        pickedLift.setWLimit(model.getLift(liftID).getWLimit());
        pickedLift.setMinFloor(model.getLift(liftID).getMinFloor());
        pickedLift.setMaxFloor(model.getLift(liftID).getMaxFloor());
        pickedLift.setFloor(model.getLift(liftID).getFloor());
        pickedLift.setDirection(model.getLift(liftID).getDirection());
        pickedLift.setDoors(model.getLift(liftID).areDoorsOpen());
